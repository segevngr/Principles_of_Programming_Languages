...
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, 
    ValuesExp, LetValuesExp, isValuesExp, isLetValuesExp, TupleBinding } from './L5-ast';
...
import { isClosure, makeClosure, Closure, Value, Tuple, isTuple, makeTuple } from "./L5-value";
...
const evalValues = (exp: ValuesExp, env: Env): Result<Value> =>
    bind(mapResult((value: CExp) => applicativeEval(value, env), exp.values), (values: Value[]) => makeOk(makeTuple(values)))

const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> => {
    const evalBindings = mapResult( (bdgs: TupleBinding) => evalTupleBinding(bdgs, env), exp.bindings );
    if(isFailure(evalBindings))
        return makeFailure("Failed to evaluate Bindings")
    else {
    // Flat the arrays of vars and vals:
    const vars = reduce ((acc: string[], cur: [string[], Tuple]) => acc.concat(cur[0]), [], evalBindings.value)
    const vals = reduce ((acc: Value[], cur: [string[], Tuple]) => acc.concat(cur[1].values), [], evalBindings.value)        
    return evalSequence(exp.body, makeExtEnv(vars, vals, env));
    }
}

const evalTupleBinding = (exp: TupleBinding, env: Env): Result<[string[],Tuple]> =>{
    const vars = map((b: VarDecl) => b.var, exp.vars);
    const vals = applicativeEval(exp.vals, env);
    if (isFailure(vals) || !isTuple(vals.value))
        return makeFailure("Values type is wrong");
    if(vars.length !== vals.value.values.length)
        return makeFailure("Var Decls and Values number does not match");
    else return makeOk([vars, vals.value]);
}

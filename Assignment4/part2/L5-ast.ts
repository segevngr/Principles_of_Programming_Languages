...
import { Result, bind, makeFailure, mapResult, makeOk, safe2, safe3, isFailure } from "../shared/result";
...
export interface TupleBinding {tag: "TupleBinding"; vars: VarDecl[], vals: CExp; }
export const makeTupleBinding = (vars: VarDecl[], vals: CExp): TupleBinding => 
    ({tag: "TupleBinding", vars: vars, vals: vals});
...
const parseValuesExp = (values: Sexp[]): Result<ValuesExp> =>
    bind(mapResult(parseL5CExp, values), (values: CExp[]) => makeOk(makeValuesExp(values)));

const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> => {
    if(isEmpty(body))
        return makeFailure('let-values Body is empty')
    else if (!isGoodBindings(bindings))
        return makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`);
    else
    {
        const parsedBindings = mapResult(parseTupleBindings, bindings);
        const parsedBody = mapResult(parseL5CExp, body);
        return safe2((bindings: TupleBinding[], body: CExp[]) => makeOk(makeLetValuesExp(bindings, body)))
                (parsedBindings, parsedBody);
    }
}

const parseTupleBindings = (bindings: [Sexp, Sexp]): Result<TupleBinding> =>
{
    const tuple = parseL5CExp(bindings[1]); 
    if(isFailure(tuple)) 
        return makeFailure("Bad Tuple");
    if(!isArray(bindings[0]))
        return makeFailure("Unexpecteed VarDecl format");
    else
        return safe2((vars: VarDecl[], vals: CExp) => makeOk(makeTupleBinding(vars, vals)))
            (mapResult(parseVarDecl, bindings[0]), parseL5CExp(bindings[1]));
}
...
// ==========================================================================
// Unparse values and let-values:
const unparseValuesExp = (vals: ValuesExp): Result<string> =>
    bind(mapResult(unparse, vals.values), (unparsedVals: string[]) => makeOk(`(values ${join(" ", unparsedVals)})`))

const unparseLetValuesExp = (letVals: LetValuesExp): Result<string> =>
    safe2((unparsedBindings: string, unparsedBody: string) => makeOk(`(let-values (${unparsedBindings}) ${unparsedBody})`))
    (unparseValuesBindings(letVals.bindings), unparseLExps(letVals.body));

const unparseValuesBindings = (bindings: TupleBinding[]): Result<string> =>
    bind(mapResult(unparseSingleBinding, bindings), (unparsedBindings: string[]) => makeOk(join(" ", unparsedBindings)));

const unparseSingleBinding = (binding: TupleBinding): Result<string> =>
    safe2((vars: string[], vals: string) => makeOk(`((${join(" ", vars)}) ${vals})`))
    (mapResult(unparseVarDecl, binding.vars), unparse(binding.vals));
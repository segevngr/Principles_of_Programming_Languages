...
export const parseTExp = (texp: Sexp): Result<TExp> =>
    (isArray(texp) && (texp.indexOf('->') === -1)) ? parseTupleTExp(texp) :
    parseNonTupleTExp(texp);

export const parseTupleTExp = (texps: Sexp[]): Result<TupleTExp> => {
    const isEmptyTuple = (texps: Sexp[]): boolean => texps[0] === 'Empty';
    // [x1 * x2 * ... * xn] => [x1,...,xn]
    ...
    bind(splitEvenOdds(texps), (cleanTup: Sexp[]) => bind(mapResult(parseNonTupleTExp, cleanTup), (cleanTup: NonTupleTExp[]) => 
                                                        makeOk(makeNonEmptyTupleTExp(cleanTup))));
}

export const parseNonTupleTExp = (texp: Sexp): Result<NonTupleTExp> =>
...
export const parseProcTExp = (texps: Sexp[]): Result<TExp[]> => {
    const isEmptyTuple = (texps: Sexp[]): boolean => texps[0] === 'Empty';
    // [x1 * x2 * ... * xn] => [x1,...,xn]
    const splitEvenOdds = (texps: Sexp[]): Result<Sexp[]> =>
        isEmpty(texps) ? makeOk([]) :
    ...
    bind(splitEvenOdds(texps), (args: Sexp[]) => mapResult(parseTExp, args));
}
...
const parseCompoundTExp = (texps: Sexp[]): Result<ProcTExp> => {
    ...
    return (pos === -1)  ? makeFailure(`Procedure type expression without -> - ${texps}`) :
           ...
           safe2((args: TExp[], returnTE: TExp) => makeOk(makeProcTExp(args, returnTE)))
                (parseProcTExp(texps.slice(0, pos)), parseTExp(texps[pos + 1]));
};
...
export const unparseTExp = (te: TExp): Result<string> => {
    const unparseTuple = (paramTes: TupleTExp): Result<string[]> =>
    isEmptyTupleTExp(paramTes) ? makeOk(["Empty"]) :
        safe2((paramTE: string, paramTEs: string[]) => makeOk([paramTE].concat(chain(te => ['*', te], paramTEs))))
            (unparseTExp(first(paramTes.TEs)), mapResult(unparseTExp, rest(paramTes.TEs)));
    
    const unparseProcParams = (paramTes: TExp[]): Result<string[]> =>
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTupleTExp(x) ? unparseTuple(x) :
        isProcTExp(x) ? safe2((paramTEs: string[], returnTE: string) => makeOk([...paramTEs, '->', returnTE]))
                            (unparseProcParams(x.paramTEs), unparseTExp(x.returnTE)) :
        makeFailure("Never");

...
import {
    ...
    Parsed, PrimOp, ProcExp, Program, StrExp, VarDecl, CExp,
    ValuesExp, LetValuesExp, isValuesExp, isLetValuesExp, TupleBinding
} from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import {
    isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
    parseTE, unparseTExp, BoolTExp, NumTExp, StrTExp, TExp, VoidTExp,
    TupleTExp, NonTupleTExp, isTupleTExp, isNonTupleTExp, makeEmptyTupleTExp,
    makeNonEmptyTupleTExp, isEmptyTupleTExp, isNonEmptyTupleTExp
} from "./TExp";
...
// Compute the types of ValuesExp:
export const typeofTupleValues = (types: TExp[]): Result<NonTupleTExp[]> =>
    mapResult((type: TExp) => isNonTupleTExp(type) ? makeOk(type) :
        makeFailure(`${type} is not a Non-Tuple`), types);

export const typeofValuesExp = (tuple: ValuesExp, tenv: TEnv): Result<TExp> => {
    if (tuple.values.length === 0)
        return makeOk(makeEmptyTupleTExp());
    else {
        const valTypes = mapResult((val: CExp) => typeofExp(val, tenv), tuple.values);
        return bind(valTypes, (types: TExp[]) => bind(typeofTupleValues(types), (tupleValTypes: NonTupleTExp[]) =>
            makeOk(makeNonEmptyTupleTExp(tupleValTypes))))
    }
};

// Compute the types of LetValuesExp:
export const typeOfLetBindings = (exp: TupleBinding, tenv: TEnv): Result<[string[], TupleTExp, TExp[]]> => {
    const vars = map((b: VarDecl) => b.var, exp.vars);
    const varTEs = map((b: VarDecl) => b.texp, exp.vars);
    const valTEs = typeofExp(exp.vals, tenv);
    if (isFailure(valTEs) || !isTupleTExp(valTEs.value))
        return makeFailure("Values type is wrong");
    else if ((isNonEmptyTupleTExp(valTEs.value) && vars.length !== valTEs.value.TEs.length))
        return makeFailure("Var Decls and Values number does not match");
    else return makeOk([vars, valTEs.value, varTEs]);
};

export const typeofLetValuesExp = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    const bindTypes = mapResult((bind: TupleBinding) => typeOfLetBindings(bind, tenv), exp.bindings);
    if (isFailure(bindTypes))
        return makeFailure("Wrong binding types");
    else {
        // To flat the arrays:
        const vars = reduce((acc: string[], curr: [string[], TupleTExp, TExp[]]) => acc.concat(curr[0]), [], bindTypes.value)
        const valTEs = reduce((acc: TExp[], curr: [string[], TupleTExp, TExp[]]) => isNonEmptyTupleTExp(curr[1]) ? acc.concat(curr[1].TEs) : acc.concat([]), [], bindTypes.value)
        const varTEs = reduce((acc: TExp[], curr: [string[], TupleTExp, TExp[]]) => acc.concat(curr[2]), [], bindTypes.value)

        const constraints = zipWithResult((varTE, valTE) => checkEqualType(varTE, valTE, exp), varTEs, valTEs);
        return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(vars, varTEs, tenv)));
    }
};

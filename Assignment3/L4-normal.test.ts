...
describe('L4 Normal Eval', () => {
    ...
    /*** Q3.3 Tests: yield different results in normal and applicative strategies ***/

    it('Q3.3 Test 1: returns 1 in normal and error in applicative', () => {
        expect(bind(parseL4(`((lambda (x) 1) (\ 2 0))`), evalNormalProgram)).to.deep.equal(makeOk(1));
    });

    it('Q3.3 Test 2: returns 10 in normal and enters infinite loop in applicative', () => {
        expect(bind(parseL4(`(define loop (lambda (x) (loop x)))
        (define g (lambda (x y) y))
        (g (loop 0) 10)`), evalNormalProgram)).to.deep.equal(makeOk(10));
    });


    it('Q3.3 Test 3: applicative triggers side effect (displays 0) while normal doesnt', () => {
        expect(bind(parseL4(`(define f (lambda (x) (display x) 3))
        (define g (lambda (x) 0)) 
        (g (f 5))`), evalNormalProgram)).to.deep.equal(makeOk(0));
    });

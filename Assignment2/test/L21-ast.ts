...
export const makeProcExp = (args: VarDecl[], body: CExp[]): ProcExp =>
    ...
    // L21
export const makeForExp = (val: VarDecl, start: NumExp, end: NumExp,body: CExp): ForExp => ({tag: "ForExp", var: val, start: start, end: end ,body: body});
...
export const parseL21CompoundCExp = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "for" ? parseForExp(params[0],params[1],params[2],params.slice(3)):
...
// <For> -> (for <VarDecl> <NumExp> <NumExp> <CExp>)
const parseForExp =(varDecl: Sexp, numStart: Sexp,numEnd: Sexp, body: Sexp[]): Result<ForExp> =>
        isIdentifier(varDecl) && isString(numStart) && isNumericString(numStart) && isString(numEnd) && isNumericString(numEnd) && +numStart <= +numEnd && body.length===1 ?
        bind(parseL21CExp(first(body)),(cexps: CExp) => makeOk(makeForExp(makeVarDecl(varDecl), makeNumExp(+numStart), makeNumExp(+numEnd),cexps))):

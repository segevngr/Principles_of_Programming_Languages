...
(define sqrt2
  (lambda (x init epsilon)
    (car (find-first (sqrt-lzl x init) (lambda (head) (< (cdr head) epsilon))))))
...
(define collect-all-values-2
  (lambda (lists key)
    (cond ((empty? lists) `())
          ((eq? (get-value$ (car lists) key (lambda(c) c) (lambda() 'fail))'fail) 
           (collect-all-values-2 (cdr lists) key))
          (else (cons (get-value$ (car lists) key (lambda(c) c) (lambda() 'fail))
                      (collect-all-values-2 (cdr lists) key))))))
